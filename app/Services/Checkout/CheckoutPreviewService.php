...
public function preview(Cart $cart): array {
   ...
   $items=[]; $subtotal=0.0;
   foreach($cart->items as $item){$variant=$item->variant;$product=$variant?->product;if(!$variant||!$product||!$product->is_active)throw ValidationException::withMessages(['cart'=>'One or more items are no longer available.']);if($variant->stock<$item->quantity)throw ValidationException::withMessages(['cart'=>"Stock changed for {$product->name}. Please review your cart."]);$unit=(float)$product->price;$line=$unit*$item->quantity;$subtotal+=$line;$items[]=[
