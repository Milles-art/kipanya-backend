...
final class CollectionController extends Controller
{
    ...
    public function update(Request $request,WearCollection $collection): RedirectResponse
    {
        ...
        if ($request->hasFile('cover_image')) { $old=$collection->cover_path; $data['cover_path']='/storage/'.$request->file('cover_image')->store('wear/collections','public'); if ($old && str_starts_with($old,'/storage/')) Storage::disk('public')->delete(ltrim(substr($old,9),'/')); }
    ...
    public function destroy(Request $request,WearCollection $collection): RedirectResponse { $this->authorize($request); $old=$collection->cover_path; $collection->products()->detach(); $collection->delete(); if($old && str_starts_with($old,'/storage/')) Storage::disk('public')->delete(ltrim(substr($old,9),'/')); app(\App\Support\AuditLogger::class)->log($request,'admin.wear.collection.deleted',null,['collection_id'=>$collection->id,'name'=>$collection->name]); return back()->with('success','Collection deleted.'); }
    private function validateData(Request $request,?WearCollection $collection=null): array { return $request->validate(['name'=>['required','string','max:180'],'slug'=>['nullable','string','max:200',Rule::unique('wear_collections','slug')->ignore($collection?->id)],'description'=>['nullable','string','max:5000'],'cover_path'=>['nullable','string','max:2048',function(string $attribute,mixed $value,$fail):void{ $value=trim((string)$value); if($value===''||str_starts_with($value,'/storage/')||str_starts_with($value,'assets/')) return; if(preg_match('/^https?:\/\//i',$value) && filter_var($value,FILTER_VALIDATE_URL)) return; $fail('The cover path must be a local storage/assets path or an HTTP(S) URL.'); }],'cover_image'=>['nullable','image','mimes:jpg,jpeg,png,webp,avif','max:5120'],'is_active'=>['sometimes','boolean'],'sort_order'=>['required','integer','min:0'],'product_ids'=>['nullable','array'],'product_ids.*'=>['integer','exists:wear_products,id']]); }
