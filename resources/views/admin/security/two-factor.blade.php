...
<div class="mx-auto max-w-2xl">
    <header class="mb-6">
        ...
        <p class="mt-1 text-sm text-black">Add a one-time code from your authenticator app to every administrator sign-in.</p>
    </header>
    ...
    <section class="rounded-3xl border border-black bg-white p-6 shadow-sm">
        @if($enabled)
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h2 class="text-lg font-bold">Enabled</h2>
                    <p class="mt-1 text-sm text-black">Every administrator sign-in for this account now requires a six-digit code.</p>
                </div>
        ...
        @else
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-black">Your account signs in with a phone code only.</p>
                </div>
                <span class="rounded-full bg-white px-3 py-1 text-xs font-bold text-black">OFF</span>
            </div>

            <div class="mt-6 rounded-2xl border border-dashed border-black p-5">
                <h3 class="text-sm font-bold">Step 1 — Add a secret to your authenticator app</h3>
                <p class="mt-1 text-sm text-black">Use Google Authenticator, Authy, 1Password, or any compatible app. Enter this secret manually or scan:</p>
                @if($uri)
                    <a href="{{ $uri }}" class="mt-3 inline-block rounded-xl bg-black px-4 py-2 text-sm font-semibold text-white">Open on this device</a>
                @endif
                @if($secret)
                    <p class="mt-4 break-all font-mono text-sm tracking-wide bg-white rounded-lg p-3">{{ $secret }}</p>
                @endif
            </div>
            ...
            <p class="mt-4 text-xs text-black">Back up your secret somewhere safe. If you lose access to your authenticator app you will need an administrator to reset it.</p>
        @endif
    </section>
</div>
@endsection