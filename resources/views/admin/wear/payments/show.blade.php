            ...
            <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-start">
                ...
                @if($needsRefund)
                    <form method="POST" action="{{ route('admin.wear.payments.refund', $payment) }}" class="flex flex-1 flex-col gap-2 sm:flex-row">
                        ...
                        @endif
                        <button class="rounded-xl bg-black px-4 py-2.5 text-sm font-bold text-white hover:bg-black" onclick="return confirm('Record this refund? This closes the reconciliation item.')">Record refund</button>
                    </form>
                @endif
            </div>
        </section>
