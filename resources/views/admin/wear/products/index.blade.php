...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-black">Manage the products shown in the Wear storefront.</p>
        </div>
    ...
    <form method="GET" class="grid gap-3 rounded-2xl border border-black bg-white p-4 sm:grid-cols-[1fr_180px_180px_auto]">
        ...
        <button class="rounded-xl border border-black px-4 py-3 text-sm font-semibold hover:bg-white">Filter</button>
    </form>
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-black">{{ $product->slug }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-4 text-black">{{ $product->category }}</td>
                        <td class="px-5 py-4 font-semibold">TZS {{ number_format((float) $product->price, 0) }}</td>
                        <td class="px-5 py-4 text-black">{{ $product->variants_count }}</td>
                        <td class="px-5 py-4">
                            <span class="rounded-full px-2.5 py-1 text-xs font-bold {{ $product->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-white text-black' }}">
                                {{ $product->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-5 py-4 text-right">
                            <a class="font-semibold text-black hover:text-emerald-700" href="{{ route('admin.wear.products.edit', $product) }}">Edit</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-black">No products found.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
        <div class="border-t border-black px-5 py-4">{{ $products->links() }}</div>
    </div>
</div>
@endsection
