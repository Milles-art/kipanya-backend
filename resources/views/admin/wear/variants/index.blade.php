...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">{{ $product->name }} · {{ $variants->total() }} variant(s)</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('admin.wear.products.edit', $product) }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold hover:bg-gray-50">Product</a>
            ...
            <table class="min-w-full text-left text-sm">
                ...
                @forelse($variants as $variant)
                    <tr class="hover:bg-gray-50/70">
                        ...
                        <td class="px-5 py-4 text-gray-600">{{ $variant->color }}</td>
                        <td class="px-5 py-4"><span class="font-bold">{{ number_format($variant->stock) }}</span></td>
                        <td class="px-5 py-4 text-right"><a class="font-semibold text-gray-900 hover:text-emerald-700" href="{{ route('admin.wear.products.variants.edit', [$product, $variant]) }}">Edit</a></td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-5 py-12 text-center text-gray-500">No variants yet.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
