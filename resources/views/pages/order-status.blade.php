...
<div data-order-status data-order-number="{{ request()->route('orderNumber') }}" class="mx-auto max-w-lg px-4 pb-20 pt-20 text-center">
    ...
    <p class="mt-6 text-xs font-semibold uppercase tracking-wider text-gray-400">KP Wear order</p>
    <h1 data-order-status-title class="mt-2 text-3xl font-bold tracking-tight text-gray-900">Order created</h1>
    <p class="mt-1 font-mono text-sm text-gray-500">{{ request()->route('orderNumber') }}</p>

    <div class="mt-7 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p data-order-status-text class="flex min-h-8 items-center justify-center gap-2 leading-7 text-gray-600" aria-live="polite">
            <span class="h-2 w-2 animate-pulse rounded-full bg-amber-500"></span>
            Loading order status…
        </p>
        <p class="mt-4 text-xs leading-5 text-gray-400">Payment must be completed before the order is confirmed. You can safely leave this page and return to your orders.</p>
    </div>
