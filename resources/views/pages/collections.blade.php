...
<div class="mx-auto max-w-7xl px-4 pb-20 pt-12 sm:px-6 lg:px-8">
  ...
  <div class="mt-8 grid gap-6 md:grid-cols-2">
    @foreach([
      ...
      <a href="{{ route('shop') }}" class="group relative aspect-[4/3] overflow-hidden rounded-3xl bg-gray-100"><img src="{{ asset('assets/wear/home/'.$image) }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" alt="{{ $name }}"><div class="absolute inset-0 bg-gradient-to-t from-black/75 to-transparent"></div><div class="absolute bottom-7 left-7 text-white"><p class="text-xs font-semibold uppercase tracking-wider text-emerald-300">Collection</p><h2 class="mt-1 text-2xl font-bold">{{ $name }}</h2><p class="mt-1 max-w-sm text-sm text-white/75">{{ $desc }}</p></div></a>
    @endforeach
  </div>
</div>
@endsection
