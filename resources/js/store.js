(() => {
  ...
  const api = async (path, options = {}) => {
    ...
    if (!token()) {
      headers.set('X-Guest-Cart-Token', ensureGuestToken());
    }
  ...
  const counts = async () => {
    ...
    const wishCount =
      document.querySelectorAll('[data-wishlist-product]').length;

    document
      .querySelectorAll('[data-wishlist-count]')
      .forEach((n) => n.classList.toggle('hidden', !wishCount));
  };
      ...
      .map(
        ...
        <article class="group min-w-0">
          <a href="/product/${encodeURIComponent(p.slug)}"
             class="relative mb-3 block aspect-[3/4] overflow-hidden rounded-2xl bg-gray-100">
            <img
              src="${p.image || ''}"
              alt="${p.name}"
            ...
            ${
              p.badge
                ? `<span class="absolute left-3 top-3 rounded-full bg-gray-950 px-2.5 py-1 text-xs font-bold text-white">${p.badge}</span>`
                : ''
            }
            ${
              p.availability === 'out_of_stock'
                ? '<span class="absolute right-3 top-3 rounded-full bg-gray-800 px-2.5 py-1 text-xs font-bold text-white">Sold out</span>'
                : ''
            }
          </a>
          ...
          <a
            ...
            class="line-clamp-1 text-sm font-medium text-gray-900 hover:text-emerald-600"
          >
            ${p.name}
          </a>
          ...
          <div class="mt-1 flex items-center gap-2">
            ...
            ${
              p.compare_at_price !== null
                ? `<span class="text-xs text-gray-400 line-through">${Number(
                    p.compare_at_price
                  ).toLocaleString()} TZS</span>`
                : ''
            }
          </div>
          ...
          <div class="mt-3 flex gap-2">
            <button
              data-wishlist-product="${p.id}"
              class="rounded-full border border-gray-200 px-3 py-1 text-xs hover:border-rose-400 hover:text-rose-500"
            >
              Save
            </button>
            ...
            <button
              ...
              class="rounded-full bg-gray-900 px-3 py-1 text-xs text-white hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-40"
            >
              Add to bag
            </button>
          </div>
  ...
  const fetchProducts = async (params = '') =>
    (await api(`/wear/products${params ? `?${params}` : ''}`))?.data || [];
          ...
          .map(
            (c) => `
              <a
                ...
                class="group rounded-2xl border border-gray-100 p-5 hover:border-emerald-200"
              >
                <p class="text-sm font-semibold">${c.name}</p>
                <p class="mt-1 text-xs text-gray-400">Shop collection</p>
              </a>
            `
          )
          .join('');
  ...
  const bootCatalog = async () => {
    ...
    try {
      ...
      if (!products.length) {
        document
          .querySelector('[data-catalog-empty]')
  ...
  const bootProduct = async () => {
    ...
    try {
      const response = await api(
        `/wear/products/${encodeURIComponent(slug)}`
      );
      ...
      page.querySelector('[data-product-category]').textContent =
        p.category || '';
      page.querySelector('[data-product-description]').textContent =
        p.description || '';
      page.querySelector('[data-product-price]').textContent =
        `${Number(p.price).toLocaleString()} TZS`;
      ...
      if (p.compare_at_price !== null) {
        compare.textContent =
          `${Number(p.compare_at_price).toLocaleString()} TZS`;
      ...
      if (sizes) {
        ...
        page.dataset.selectedVariant =
          findFirstVariant(p)?.id || '';
      }

      page
        .querySelector('[data-add-selected]')
        ?.addEventListener('click', async () => {
          if (!page.dataset.selectedVariant) {
            return toast('Select an available size');
          }
  ...
  const bootCart = async () => {
    ...
    const render = async () => {
            ...
            <div class="flex gap-4 border-b border-gray-100 py-5">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-3 flex items-center gap-3">
                  <button
                    data-cart-minus="${item.variant_id}"
                  ...
                  <button
                    data-cart-plus="${item.variant_id}"
                    class="rounded border px-2"
                  >
                    +
                  </button>
                  ...
                  <button
                    ...
                    class="ml-3 text-sm text-rose-500"
                  >
                    Remove
                  </button>
                </div>
      ...
      page
        ...
        ?.classList.toggle('hidden', cart.items.length > 0);

      page
        .querySelector('[data-cart-content]')
        ?.classList.toggle('hidden', cart.items.length === 0);

      page.querySelector('[data-cart-summary]').textContent =
        `${Number(cart.subtotal).toLocaleString()} TZS`;
    ...
    page.addEventListener('click', async (e) => {
      const b = e.target.closest(
        '[data-cart-plus],[data-cart-minus],[data-cart-remove]'
      );
      ...
      try {
        const variant =
          b.dataset.cartPlus ||
          b.dataset.cartMinus ||
          b.dataset.cartRemove;

        const cart = (await api('/cart')).data;

        const item = cart.items.find(
          (i) => String(i.variant_id) === String(variant)
        );
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (first) {
          ...
          toast('Verification code requested');
      ...
      if (add) {
        try {
          ...
          const p = products.find(
            (x) => String(x.id) === String(add.dataset.quickAdd)
          );

          const v = findFirstVariant(p);

          if (!v) {
            throw new Error('No variant available');
          }

          await addVariantToCart(v.id);
        } catch (err) {
          toast(err.message);
        }
      }

      const wish = e.target.closest('[data-wishlist-product]');

      if (wish) {
        try {
          await toggleWishlist(
            wish.dataset.wishlistProduct,
            wish
          );
        } catch (err) {
          toast(err.message);
  ...
  const bootCheckout = async () => {
    ...
    if (!token()) {
      page
        .querySelector('[data-checkout-auth]')
        ?.classList.remove('hidden');

      page
        .querySelector('[data-address-form]')
        ?.classList.add('hidden');

      page
        .querySelector('[data-place-order]')
    ...
    const renderAddresses = (addresses) => {
      addressList.innerHTML = addresses.length
        ? addresses
            ...
            .join('')
        : '<p class="text-sm text-gray-500">No saved addresses yet. Add one below.</p>';
    ...
    try {
      ...
      summary.innerHTML = `
        ...
        <div class="mt-3 flex justify-between border-t border-gray-200 pt-3 text-base">
          <span>Total</span>
          <strong>${Number(
            d.total || d.subtotal || 0
          ).toLocaleString()} TZS</strong>
        </div>
      `;
    } catch (e) {
    ...
    page
      ...
      ?.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          selectedAddress = payload.data?.id || null;
    ...
    page
      ...
      ?.addEventListener('click', async () => {
        if (!selectedAddress) {
          return toast('Add or select a delivery address');
        }

        const key = `kp-${Date.now()}-${crypto
          .randomUUID()
          .replaceAll('-', '')
          .slice(0, 16)}`;
        ...
        try {
          const order = await api('/checkout', {
            ...
            body: {
              ...
              notes:
                page.querySelector('[data-order-notes]')?.value ||
                null,
            },
          });

          location.href =
            `/orders/${order.data.order_number}`;
        } catch (e) {
          toast(e.message);
        }
      });
  ...
  const bootAccount = async () => {
    ...
    const sidebar = document.querySelector('aside [data-sidebar-name]')
    ...
    if (!token()) {
      if (account) location.href = '/login';
      return;
    }
    ...
    try {
      ...
      document
        ...
        .forEach(
          (n) =>
            (n.textContent = `Welcome back, ${me.data.name}`)
        );

      document
        .querySelectorAll('[data-profile-name]')
        .forEach((n) => (n.textContent = me.data.name));

      document
        .querySelectorAll('[data-profile-phone]')
        .forEach((n) => (n.textContent = me.data.phone));

      document
        .querySelectorAll('[data-sidebar-name]')
        .forEach((n) => (n.textContent = me.data.name));

      document
        .querySelectorAll('[data-sidebar-phone]')
        .forEach((n) => (n.textContent = me.data.phone));
      ...
      if (account) {
        ...
        ).textContent = orders.data?.length ?? 0;
      }
    } catch (e) {
      toast(e.message);
    }
          ...
          .map(
            (o) => `
              <div class="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-gray-100 p-5">
                ...
                <strong>
                  ${Number(o.total).toLocaleString()} TZS
                </strong>
      ...
      page.querySelector(
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-1 font-semibold">${o.status}</p>
          </div>

          <div>
            <p class="text-sm text-gray-500">Payment</p>
            <p class="mt-1 font-semibold">${o.payment_status}</p>
          </div>
            ...
            .map(
              (i) => `
                <div class="flex justify-between border-b border-gray-100 py-3 text-sm">
                  ...
                  <strong>
                    ${Number(i.line_total).toLocaleString()} TZS
                  </strong>
                </div>
              `
            )
  ...
  const bootAddresses = async () => {
    ...
    try {
      ...
      page.querySelector('[data-addresses-list]').innerHTML =
        (data.data || [])
          .map(
            (a) => `
              <div class="rounded-2xl border border-gray-100 p-5">
                <div class="flex items-center justify-between">
                  ...
                  ${
                    a.is_default
                      ? '<span class="text-xs font-semibold text-emerald-700">Default</span>'
                      : ''
                  }
                </div>
